...
exports.logs = function(req, res) {
    ...
    if (req.query.task == 'del'){
        ...
        pool.getConnection(function(err,connection){
            ...
            var logs = Array();
            connection.query('SELECT l.lid lid, date_format(l.timestamp, \'%Y-%m-%d %h:%i:%s.000\') timestamp, t.name tag, e.name event, m.name machine, a.title article, l.remarks remarks, l.iid iid FROM logs l LEFT JOIN machines m ON m.mid=l.mid LEFT JOIN articles a ON a.aid=l.aid LEFT JOIN tags t ON t.tid=l.tid LEFT JOIN events e ON e.eid=l.eid ORDER BY l.timestamp', function(error, rows, fields) {
                if (error) {
                    res.send(error);
                } else {
                    logs = rows;
    ...
    if (req.query.mid){
        if (!req.body.name){
            ...
            pool.getConnection(function(err,connection){
                ...
                connection.query('UPDATE machines SET name=?, config=?, price=?, period=?, uid=?, min_periods=?, offdelay=?, watchdog=? WHERE mid=?', [req.body.name, req.body.config, req.body.price, req.body.period, req.body.uid, req.body.min_periods, req.body.nachlauf, req.body.watchdog, req.query.mid], function(error, result) {
                    if (error) {
                        res.send(error);
                    } else {
        ...
        if (!req.body.name){
            ...
            pool.getConnection(function(err,connection){
                ...
                connection.query('INSERT machines SET name=?, config=?, price=?, period=?, uid=?, min_periods=?, offdelay=?, watchdog=?', [req.body.name, req.body.config, req.body.price, req.body.period, req.body.uid, req.body.min_periods, req.body.nachlauf, req.body.watchdog], function(error, result) {
                    if (error) {
                        res.send(error);
                    } else {
                        res.redirect('/machines');
